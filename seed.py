...
with app.app_context():
    ...
    # Senaryo 3: Sınırda Kullanıcı (Teslim tarihine 1 gün var)
    user3 = User(username="batu",
                 email_address="batu@mail.com",
                 password="123",
                 budget=100,
                 is_approved=True)
    db.session.add_all([admin_user, user1, user2, user3])
    db.session.commit()
    print("✓ Kullanıcılar oluşturuldu (admin/admin123, ahmet/123456, ayse/123456, batu/123)")
    ...
    # Sınırda Kitap (14 gün önce alınmış, yarına bitiyor)
    borrow_soon = Borrow(
        user_id=user3.id,
        book_id=books[1].id,
        borrow_date=now - timedelta(days=14),
        due_date=now + timedelta(days=1),  # YARIN BİTİYOR
        fine_amount=0.0
    )
    ...
    db.session.add_all([borrow1, borrow2, borrow3, borrow_soon])
    db.session.commit()
    ...
    print("  Kullanıcı 1: batu / 123")
    print("  Kullanıcı 2: ahmet / 123456")
    ...
    print(f"  - {len([admin_user, user1, user2,user3])} Kullanıcı")
